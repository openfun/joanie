...
class InvoiceModelTestCase(BaseAPITestCase):
    ...
    def test_models_invoice_property_can_generate_final_reference_order_cpf(self):
        """
        The property `can_generate_final_reference` should return False when the
        nature of the order is `CPF` and is `¢ompleted`
        """
        order = OrderGeneratorFactory(
            state=enums.ORDER_STATE_COMPLETED, nature=enums.ORDER_NATURE_CPF
        )

        self.assertFalse(order.main_invoice.can_generate_final_reference)

    def test_models_invoice_property_can_generate_final_reference_order_from_batch_order(
        self,
    ):
        """
        The property `can_generate_final_reference` should return False when the
        nature of the order is `B2B` and is `completed.
        """
        order = OrderGeneratorFactory(
