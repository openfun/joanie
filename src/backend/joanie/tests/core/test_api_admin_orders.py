...
class OrdersAdminApiTestCase(TestCase):
    ...
    def test_api_admin_orders_cancel_an_order(self):
        """
        Authenticated admin users should be able to cancel an order.
        The order's state should be `canceled` after calling the endpoint.
        """
        ...
        for state, _ in order_state_choices:
            with self.subTest(state=state):
                ...
                response = self.client.post(
                    f"/api/v1.0/admin/orders/{order.id}/cancel/"
                )
