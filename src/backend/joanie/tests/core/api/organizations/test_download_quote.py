...
class OrganizationApiDownloadQuoteTest(BaseAPITestCase):
    ...
    def test_api_organization_download_quote_with_organization_access_not_owner_or_admin_role(
        self,
    ):
        """
        Authenticated user with organization access that is neither owner nor admin role should
        not be able to download the quote document.
        """
        batch_order = factories.BatchOrderFactory(state=enums.BATCH_ORDER_STATE_QUOTED)

        # Check every existing roles except 'owner' or 'admin'
        for role in [
            role[0]
