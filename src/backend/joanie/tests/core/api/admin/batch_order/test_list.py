...
class BatchOrdersAdminApiListTestCase(BaseAPITestCase):
    ...
    def test_api_admin_batch_orders_list_filter_by_query(self):
        ...
        order = factories.BatchOrderFactory(
    ...
    def test_api_admin_batch_orders_list_filter_by_ids(self):
        ...
        orders = factories.BatchOrderFactory.create_batch(3)

        admin = factories.UserFactory(is_staff=True, is_superuser=True)
        self.client.login(username=admin.username, password="password")

        response = self.client.get(
            f"/api/v1.0/admin/batch-orders/?ids={orders[0].id}&ids={orders[1].id}"
        )
        self.assertStatusCodeEqual(response, HTTPStatus.OK)
        content = response.json()
        self.assertEqual(content["count"], 2)
        self.assertCountEqual(
            ...
            [str(orders[0].id), str(orders[1].id)],
        )
