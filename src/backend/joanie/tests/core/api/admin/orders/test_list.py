...
class OrdersAdminApiListTestCase(TestCase):
    ...
    def test_api_admin_orders_list_filter_by_created_on_no_result_because_order_were_not_created(
        self,
    ):
        """
        Authenticated admin user should be able to get a list of orders by filtering
        with the `created_on` field. When no orders are found, it should not return any result.
        """
        admin = factories.UserFactory(is_staff=True, is_superuser=True)
        self.client.login(username=admin.username, password="password")
    ...
    def test_api_admin_order_list_filter_by_created_on_before_no_result(self):
        """
        Authenticated admin user should find no result if he searches for
        order that were created before a certain data when no order was yet created
        at that time.
        """
        admin = factories.UserFactory(is_staff=True, is_superuser=True)
        self.client.login(username=admin.username, password="password")
    ...
    def test_api_admin_orders_list_filter_by_created_on_after_no_result(self):
        """
        Authenticated admin user should not be able to get orders before no
        orders were created after the given date. He should get no result.
        """
        admin = factories.UserFactory(is_staff=True, is_superuser=True)
        self.client.login(username=admin.username, password="password")
    ...
    def test_api_admin_order_list_filter_by_created_on_after(self):
        ...
        were created after the given date.
        """
        admin = factories.UserFactory(is_staff=True, is_superuser=True)
        self.client.login(username=admin.username, password="password")
    ...
    def test_api_admin_order_list_filter_by_created_on_before_and_after_date_gets_no_result(
        ...
        """
        Authenticated admin user should not be able to find orders because the
        range of given date do not match any orders created.
        """
        admin = factories.UserFactory(is_staff=True, is_superuser=True)
        self.client.login(username=admin.username, password="password")
    ...
    def test_api_admin_order_list_filter_by_created_on_before_and_after(self):  # pylint: disable=too-many-locals
        """
        Authenticated admin user should be able to find orders because the
        range of given date match orders that were created.
        """
        admin = factories.UserFactory(is_staff=True, is_superuser=True)
        self.client.login(username=admin.username, password="password")
    ...
    def test_api_admin_order_list_filter_by_created_on_date_range_only_with_after_date(
        ...
        """
        Authenticated admin user should be able to get the list of orders created after
        the given date with the filter `created_on_date_rage`. It takes the orders where
        the created_on field is equal to the given date and in the future from that starting
        point.
        """
        admin = factories.UserFactory(is_staff=True, is_superuser=True)
        self.client.login(username=admin.username, password="password")
    ...
    def test_api_admin_order_list_filter_by_created_on_date_range_only_with_before_date(
        ...
        """
        Authenticated admin user should be able to get the list of orders created before
        the given date with the filter `created_on_date_rage`. It takes the orders where
        the created_on field is equal to the given date and in the past from that starting
        point.
        """
        admin = factories.UserFactory(is_staff=True, is_superuser=True)
        self.client.login(username=admin.username, password="password")
