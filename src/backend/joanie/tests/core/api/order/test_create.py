...
class OrderCreateApiTest(BaseAPITestCase):
    ...
    def test_api_order_create_authenticated_product_course_unicity_state_is_refunding(
        self,
    ):
        """
        When a user tries to create a new order with a triplet product-course-owner and a same
        order already exists that is in `refunding` state, it should create it.
        """
        user = factories.UserFactory()
        token = self.generate_token_from_user(user)
    ...
    def test_api_order_create_authenticated_product_course_unicity_state_is_refunded(
        self,
    ):
        """
        When a user tries to create a new order with a triplet product-course-owner and a same
        order already exists that is in `refunded` state, it should create it.
        """
        user = factories.UserFactory()
        token = self.generate_token_from_user(user)
    ...
    def test_api_order_create_authenticated_product_enrollment_unicity_state_is_refunding(
        self,
    ):
        """
        When a user tries to create a new order with a triplet product-enrollment-owner and a same
        order already exists that is in `refunding` state, it should create it.
        """
        user = factories.UserFactory()
        token = self.generate_token_from_user(user)
    ...
    def test_api_order_create_authenticated_product_enrollment_unicity_state_is_refunded(
        self,
    ):
        """
        When a user tries to create a new order with a triplet product-enrollment-owner and a same
