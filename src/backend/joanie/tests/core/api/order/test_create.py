...
class OrderCreateApiTest(BaseAPITestCase):
    ...
    @mock.patch("joanie.core.models.products.Order.enroll_user_to_course_run")
    def test_api_order_create_with_voucher_code_after_individual_order_canceled(
        self, mock_enroll_user_to_course_run
    ):
        """
        When a user claims an order from a batch order with a voucher code,
        then the individual order is canceled, the user should not be able
        to re-use the same voucher code to create a new B2C order on the
        same product. The voucher code should no longer be usable.
        """
        user = factories.UserFactory()
        token = self.generate_token_from_user(user)
        ...
        # User claims the first order with the first voucher code
        order = batch_order.orders.get(voucher__code=voucher_codes[0])
        order.owner = user
        order.flow.update()
        order.save()
        ...
        # Cancel the individual order (not the whole batch order)
        order.flow.cancel()

        # User attempts to re-use the same voucher code to create a new B2C order
        # with all required fields so we don't get a validation error before
        # reaching the voucher check
        data = {
            "product_id": batch_order.offering.product.id,
            "course_code": batch_order.offering.course.code,
            "voucher_code": voucher_codes[0],
        ...
        # The voucher code should not allow creating a new B2C order
        mock_enroll_user_to_course_run.assert_not_called()
        self.assertStatusCodeEqual(response, HTTPStatus.BAD_REQUEST)

    @mock.patch("joanie.core.models.products.Order.enroll_user_to_course_run")
    def test_api_order_create_with_voucher_code_from_fully_canceled_batch_order(
        self, mock_enroll_user_to_course_run
    ):
        """
        When a user attempts to use a voucher code that was generated by
        a batch order that got canceled, he should not be able to create a
        B2C order with it. The voucher code should no longer be usable.
        """
        user = factories.UserFactory()
        token = self.generate_token_from_user(user)
        ...
        # User attempts to use the voucher code to create a new B2C order
        # with all required fields so we don't get a validation error before
        # reaching the voucher check
        data = {
            "product_id": batch_order.offering.product.id,
    ...
    def test_api_order_create_with_voucher_code_from_batch_order_enrolls_to_course_run(
        self, _mock_thumbnail
    ):
        """
        When an authenticated user uses a voucher code from a batch order to claim an order,
        the method `enroll_user_to_course_run` should be called and we should see in
        the response the target enrollments and target courses data.
        """
        user = factories.UserFactory()
        token = self.generate_token_from_user(user)
        ...
        self.assertEqual(
            ...
            response.json()["target_enrollments"],
        )
