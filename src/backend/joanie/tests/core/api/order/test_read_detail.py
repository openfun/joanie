...
class OrderReadApiTest(BaseAPITestCase):
    ...
    def test_api_order_read_detail_authenticated_organization_owner(
        self, _mock_thumbnail
    ):
        """Organization owners should be allowed to retrieve an order."""
        organization_owner = factories.UserFactory()
        organization = factories.OrganizationFactory()
        factories.UserOrganizationAccessFactory(
            user=organization_owner, organization=organization, role=enums.OWNER
        )
        *target_courses, _other_course = factories.CourseFactory.create_batch(
        ...
        order = factories.OrderFactory(
            course=course, product=product, organization=organization
        )
        token = self.generate_token_from_user(organization_owner)
