...
class SignalsTestCase(TestCase):
    ...
    def test_signals_on_save_course_run_restrict(self, mock_sync):
        """
        When a course run restriction is in place, the submit_hook should
        ...
        # the submit_hook only concerns the course run, no data for the product are sent
        self.assertCountEqual(
            synchronized_course_runs,
    ...
    def test_signals_on_delete_course_run_object(
        ...
        """
        submit_hook should be triggered when a course run is deleted
        setting the visibility to hidden for the course run.
        A second call should be made for the related product's equivalent course.
        """
