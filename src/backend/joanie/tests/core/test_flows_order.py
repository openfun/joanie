...
class OrderFlowsTestCase(TestCase, BaseLogMixinTestCase):
    ...
    def test_flows_order_save_payment_method_to_pending_mail_sent_confirming_subscription(
        ...
        """
        Test that the post transition success action when the pending transition
        is successful when the order flow goes from from TO_SAVE_PAYMENT_METHOD
        to PENDING, it should trigger the email confirmation the subscription that is sent
        to the user.
        """
        for state in [
            enums.ORDER_STATE_TO_SAVE_PAYMENT_METHOD,
            ...
            with self.subTest(state=state):
                ...
                order = factories.OrderFactory(state=state, owner=user)
                order.flow.pending()
                self.assertEqual(order.state, enums.ORDER_STATE_PENDING)
    ...
    def test_flows_order_signing_to_pending_mail_sent_confirming_subscription(self):
        """
        Test that the post transition success action when the pending transition
        is successful when the order flow goes from from SIGNING to PENDING, it should trigger
        the email confirmation the subscription that is sent to the user.
        """
        for state in [
            enums.ORDER_STATE_SIGNING,
        ]:
