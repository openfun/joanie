...
class BatchOrderModelsTestCase(LoggingTestCase):
    ...
    def test_models_batch_order_can_submit_for_signature_not_transition_completed_called_twice(
        self,
    ):
        """
        When submit_for_signature is called twice on a batch order using the purchase order
        payment method, the order should not transition to completed until the buyer signs
        the contract.
    ...
    def test_models_batch_order_can_generate_orders_not_completed(self):
        """
        When the batch order is not in COMPLETED state,
        can_generate_orders should return False.
        """
        batch_order = factories.BatchOrderFactory(
            state=enums.BATCH_ORDER_STATE_PENDING,
            payment_method=enums.BATCH_ORDER_WITH_BANK_TRANSFER,
    ...
    def test_models_batch_order_can_generate_orders_orders_already_generated(self):
        """
        When the batch order already has orders generated,
        can_generate_orders should return False.
        """
        batch_order = factories.BatchOrderFactory(
            state=enums.BATCH_ORDER_STATE_COMPLETED,
        ...
        # Orders are already generated by the factory in COMPLETED state
