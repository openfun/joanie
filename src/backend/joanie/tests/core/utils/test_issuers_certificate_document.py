...
def generate_certificate(template: str, dual_issuers: bool = False):
    """Generate a certificate with the given template and optional dual issuers."""
    organizations = [
...
class UtilsIssuersCertificateGenerateDocumentTestCase(TestCase):
    ...
    def test_utils_issuers_generate_document_certificate_degree_document(self):
        ...
        self.assertIn(
            certificate.issued_on.strftime("%m/%d/%Y"), document_text.replace(" ", "")
        )
    ...
    def test_utils_issuers_generate_document_microcredential_degree_default_dual_issuers(
        ...
        """
        certificate = generate_certificate(
            enums.MICROCREDENTIAL_DEGREE_DEFAULT, dual_issuers=True
        )
        product = certificate.order.product
        ...
        self.assertRegex(
            document_text, r"Joanie Cunningham.*Head of the department.*University X"
        )
        self.assertRegex(
            document_text, r"Jane Doe.*Head of the department.*University Y"
        )
        self.assertRegex(document_text, r"en-us")
    ...
    def test_utils_issuers_verify_document_microcredential_degree_default_dual_issuers_style(
        ...
        original_image = Image.open(base_path + file_name + "_original.png").convert(
            "RGB"
        )
    ...
    def test_utils_issuers_generate_document_microcredential_degree_unicamp_dual_issuers(
        ...
        """
        certificate = generate_certificate(
            enums.MICROCREDENTIAL_DEGREE_UNICAMP, dual_issuers=True
        )
        product = certificate.order.product
