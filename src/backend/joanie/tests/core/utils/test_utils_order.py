...
class UtilsOrderTestCase(TestCase):
    ...
    def test_utils_verify_voucher(self):
        """
        The method `verify_voucher` will if the passed voucher code exists and is active into
        our database before letting a user use it. It returns the voucher if found, else it returns
        None.
        """
    ...
    def test_utils_get_order_batch_order(self):
        """
        The method `get_order_batch_order` should return the order in state `to_own` that was
