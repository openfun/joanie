...
class InvoiceModelTestCase(TestCase):
    ...
    def test_models_invoice_credit_note_with_amount_greater_than_related_invoice_amount(
        ...
        self.assertEqual(
            ...
            "{'__all__': ['Credit note amount cannot be greater than its related invoice invoiced balance.']}",  # noqa pylint: disable=line-too-long
        )
    ...
    def test_models_invoice_balance(self):
        """
        Invoice.balance should return the current balance of the invoice.
        """
        ...
        TransactionFactory(total=Money(-100.00, "XYZ"), invoice=credit_note)
        invoice.refresh_from_db()
        self.assertEqual(invoice.balance.amount, D("0.00"))
    ...
    def test_models_invoice_state_refunded(self):
        """
        If invoice balance is greater or equal than zero, transactions_balance
        and invoiced_balance are equal to zero, invoice state should be refunded
        """
        product = ProductFactory(price="100.00")
        order = OrderFactory(product=product)
        invoice = InvoiceFactory(order=order, total=order.total)
    ...
    def test_models_invoice_state_unpaid(self):
        """
        If invoice balance is less than zero, invoice state should be unpaid.
        """
        product = ProductFactory(price="100.00")
        order = OrderFactory(product=product)
        invoice = InvoiceFactory(order=order, total=order.total)
    ...
    def test_models_invoice_child_cannot_relies_on_another_child(self):
        """
        An invoice cannot have a parent which is a child of another invoice.
        """
        order = OrderFactory()
        invoice = InvoiceFactory(order=order, total=order.total)
        ...
        with self.assertRaises(ValidationError) as context:
            InvoiceFactory(order=child_invoice.order, parent=child_invoice, total=20.00)

        self.assertEqual(
            str(context.exception),
            "{'__all__': ['Invoice cannot have as parent another invoice which is a child.']}",
        )
    ...
    def test_models_invoice_localized_context(self):
        """
        When an invoice is created, localized contexts in each enabled languages
        should be created.
        """
        order = OrderFactory()
    ...
    def test_models_invoice_document_type_credit_note(self):
        """
        If invoice type is "credit_note", a document of type credit_note should
        be generated.
        """
        product = ProductFactory()
        order = OrderFactory(product=product)
        ...
        credit_note = InvoiceFactory(
            order=invoice.order, parent=invoice, total=-invoice.total
        )
