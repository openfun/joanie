...
class PayplugBackend(BasePaymentBackend):
    ...
    def save_credit_card(self, request, order, billing_address):
        """
        Save a credit card without doing a real payment in order to save the token generated by
        payplug. This token value will be saved into the credit card's token field.
        """
        payment_data = self._get_payment_data(request, order, billing_address)
        # either 'amount' or 'authorized_amount' should be declared
        del payment_data["amount"]
        payment_data["authorized_amount"] = 100
        payment_data["auto_capture"] = False
        payment_data["force_3ds"] = True
        payment_data["save_card"] = True

        try:
            payment_response = payplug.Payment.create(**payment_data)
        except BadRequest as error:
            raise exceptions.CreatePaymentFailed(str(error)) from error

        self._save_card_for_user(order=order, payment_response=payment_response)
