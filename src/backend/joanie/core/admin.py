...
def summarize_certification_to_user(request, count):
    """
    Display a message after issue_certificates command has been launched
    """
    if count == 0:
        messages.warning(
            request,
...
class IssuedCertificateAdmin(admin.ModelAdmin):
    """Admin class for the IssuedCertificate model"""

    list_display = ("order", "issued_on")
    readonly_fields = ("order", "issued_on", "certificate")

    def certificate(self, obj):  # pylint: disable=no-self-use
        """Retrieve the certification from the related order."""
        certificate = obj.order.product.certificate

        url = reverse(
            "admin:core_certificate_change",
            args=(certificate.id,),
        )
        return format_html(f"<a href='{url:s}'>{certificate!s}</a>")
...
class CourseAdmin(DjangoObjectActions, TranslatableAdmin):
    ...
    def issue_certificates(self, request, queryset):  # pylint: disable no-self-use
        """
...
class ProductAdmin(
    ...
    def get_change_actions(self, request, object_id, form_url):
        """
        Remove the issue_certificates action from list of actions
        ...
        if related_courses:
            ...
            for course in obj.courses.all():
                ...
                if is_certifying:
                    ...
                    raw_html += (
                        f'<a style="margin-left: 1rem" class="button" href="{issue_certificates_url}">'  # noqa pylint: disable=line-too-long
