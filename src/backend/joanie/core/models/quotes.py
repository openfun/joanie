...
class Quote(BaseModel):
    ...
    def clean(self):
        """
        When the object is created, add the a unique reference to it
        """
        if not self.reference:
            year = timezone.now().year
            with transaction.atomic():
                # Select for update that will lock rows of the queryset until
                ...
                if last and last.reference:
                    ...
                    last_number = 0
                # Increment last found number by one
                next_number = last_number + 1
                self.reference = f"FUN_{year}_{next_number:07d}"
