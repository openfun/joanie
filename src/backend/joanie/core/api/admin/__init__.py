...
class OrderViewSet(
    ...
    def _get_pending_withdrawal_error(self, order):
        """
        Return an error Response if the order isn't a certificate order currently
        pending withdrawal review, otherwise None.
        """
    ...
    def confirm_withdrawal(self, request, pk=None):  # pylint:disable=unused-argument
        ...
        if error_response := self._get_pending_withdrawal_error(order):
