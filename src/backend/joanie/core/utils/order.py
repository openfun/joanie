...
def verify_voucher(voucher_code: str) -> models.Voucher | None:
    """
    When a voucher code is used to create an order, we will very if it exists and whether it's
...
def get_order_batch_order(
    course_code: UUID, product_id: UUID, voucher_code: str
) -> models.Order | None:
    """
